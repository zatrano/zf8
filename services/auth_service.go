...
type IAuthService interface {
	...
	CreateUser(ctx context.Context, user *models.User) error // Added CreateUser method
	SendPasswordResetLink(account string) error
	ResetPassword(token, newPassword string) error
	VerifyEmail(token string) error // Added VerifyEmail method
...
func (s *AuthService) SendPasswordResetLink(account string) error {
	user, err := s.repo.FindUserByEmail(account)
	if err != nil {
	...
	accountBody := "Şifrenizi sıfırlamak için aşağıdaki bağlantıya tıklayın: " + resetLink

	if err := mailService.SendMail(user.Account, "Şifre Sıfırlama", accountBody); err != nil {
